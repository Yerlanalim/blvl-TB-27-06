...
const isActive = true;`} language="javascript" filename="javascript-1.js" />

### 2. Follow camelCase Convention

In JavaScript, we typically use camelCase for variables and functions:

<CodeSnippet code={`// Correct camelCase usage
...
const firstname = 'John';     // lowercase`} language="javascript" filename="javascript-2.js" />

### 3. Boolean Variables

Boolean variables should ask a question:

<CodeSnippet code={`// Good boolean names
...
const login = true;
const status = false;
const edit = true;`} language="javascript" filename="javascript-3.js" />

## Function Naming Conventions

Functions should be named using verbs that describe their action:

<CodeSnippet code={`// Good function names
...
function data() { }
function email() { }
function total() { }`} language="javascript" filename="javascript-4.js" />

### Event Handler Naming

For event handlers, use the prefix 'handle' or 'on':

<CodeSnippet code={`// React/Event handler naming
...
<CodeSnippet code={`// Good class names
class UserProfile {
...
<CodeSnippet code={`const MAX_ITEMS = 100;
const API_BASE_URL = 'https://api.example.com';
const DEFAULT_THEME = 'light';`} language="javascript" filename="javascript-7.js" />

### 2. Private Variables

Use an underscore prefix for private variables:

<CodeSnippet code={`class User {
  constructor() {
...
<CodeSnippet code={`// Good array names
...
const userName = ['John', 'Jane', 'Bob'];`} language="javascript" filename="javascript-9.js" />

## Practice Examples

Let's test your understanding with some practical examples:

<CodeSnippet code={`// Refactor these names to follow conventions
const n = 'John Smith';
const a = 25;
const p = true;
function calc(x, y) {
...
Ready to master JavaScript and write cleaner, more professional code? Join BizLevel for daily coding challenges, personalized learning paths, and a supportive community of developers. Start your journey today at <a href="https://bizlevel.dev/signup" rel="noopener noreferrer" className="text-accent">BizLevel</a>.
