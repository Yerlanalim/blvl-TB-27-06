...
export async function generateMetadata(): Promise<Metadata> {
  try {
    const { frontmatter } = await getBlogPost('how-to-create-a-weather-app-in-javascript');
    ...
    return createMetadata({
      ...
      canonicalUrl: `/javascript-projects-for-beginners/how-to-create-a-weather-app-with-javascript`,
    });
  } catch {
    return {
...
export default async function HowToCreateAWeatherAppWithJavaScript({ params }: BlogPostParams) {
  try {
    const { content, frontmatter } = await getBlogPost('how-to-create-a-weather-app-in-javascript');
    const typedFrontmatter = frontmatter as unknown as BlogFrontmatter;
    ...
    return (
      <BlogPostLayout
        ...
        userCount={userCount}
        backLink={{
          href: '/javascript-projects-for-beginners',
          text: 'Back to JavaScript Projects for Beginners',
        }}
      />
    );
  } catch (error) {
