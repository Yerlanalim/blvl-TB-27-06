...
export async function generateMetadata(): Promise<Metadata> {
  try {
    const { frontmatter } = await getBlogPost('programming-challenges-for-beginners');
    ...
    return createMetadata({
      ...
      canonicalUrl: `/javascript-projects-for-beginners/programming-challenges-for-beginners`,
    });
  } catch {
    return {
...
export default async function JavascriptProgrammingChallengesForBeginners({
  params,
}: BlogPostParams) {
  try {
    const { content, frontmatter } = await getBlogPost('programming-challenges-for-beginners');
      ...
      <div className="container flex flex-col md:flex-row gap-10 max-w-7xl mx-auto pt-32 pb-20">
        <article className="w-full md:w-3/5">
          ...
          <div className="mb-8">
            ...
            >
              <ChevronLeft size={16} />
              Back to JavaScript Projects for Beginners
            </Link>
