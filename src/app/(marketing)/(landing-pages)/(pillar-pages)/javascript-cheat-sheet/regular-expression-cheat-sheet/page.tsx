...
export async function generateMetadata(): Promise<Metadata> {
  try {
    const { frontmatter } = await getBlogPost('regular-expression-cheat-sheet');
    ...
    return createMetadata({
      ...
      canonicalUrl: `/javascript-cheat-sheet/regular-expression-cheat-sheet`,
    });
  } catch {
    return {
...
export default async function JavascriptRegularExpressionCheatSheet({ params }: BlogPostParams) {
  try {
    const { content, frontmatter } = await getBlogPost('regular-expression-cheat-sheet');
