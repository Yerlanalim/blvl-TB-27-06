...
export async function generateMetadata(): Promise<Metadata> {
  try {
    const { frontmatter } = await getBlogPost('how-to-write-loops-in-javascript');
    ...
    return createMetadata({
      ...
      canonicalUrl: `/javascript-fundamentals/how-to-write-loops-in-javascript`,
    });
  } catch {
    return {
...
export default async function HowToWriteLoopsInJavascript({ params }: BlogPostParams) {
  try {
    const { content, frontmatter } = await getBlogPost('how-to-write-loops-in-javascript');
    const typedFrontmatter = frontmatter as unknown as BlogFrontmatter;
