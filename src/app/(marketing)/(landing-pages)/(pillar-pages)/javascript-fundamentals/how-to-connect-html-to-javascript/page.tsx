...
export async function generateMetadata(): Promise<Metadata> {
  try {
    const { frontmatter } = await getBlogPost('how-to-connect-html-to-javascript');
    ...
    return createMetadata({
      ...
      canonicalUrl: `/javascript-fundamentals/how-to-connect-html-to-javascript`,
    });
  } catch {
    return {
...
export default async function HowToConnectHtmlToJavascript({ params }: BlogPostParams) {
  try {
    const { content, frontmatter } = await getBlogPost('how-to-connect-html-to-javascript');
    const typedFrontmatter = frontmatter as unknown as BlogFrontmatter;
