...
export async function generateMetadata(): Promise<Metadata> {
  try {
    const { frontmatter } = await getBlogPost('javascript-format-strings-with-variables');
    ...
    return createMetadata({
      ...
      canonicalUrl: `/javascript-fundamentals/javascript-format-strings-with-variables`,
    });
  } catch {
    return {
...
export default async function JavaScriptFormatStringsWithVariables({ params }: BlogPostParams) {
  try {
    const { content, frontmatter } = await getBlogPost('javascript-format-strings-with-variables');
