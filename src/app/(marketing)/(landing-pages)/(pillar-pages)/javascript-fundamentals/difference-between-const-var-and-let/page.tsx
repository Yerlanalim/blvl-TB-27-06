...
export async function generateMetadata(): Promise<Metadata> {
  try {
    const { frontmatter } = await getBlogPost('difference-between-const-var-and-let');
    ...
    return createMetadata({
      ...
      canonicalUrl: `/javascript-fundamentals/difference-between-const-var-and-let`,
    });
  } catch {
    return {
...
export default async function JavascriptDifferenceBetweenConstVarAndLet({
  params,
}: BlogPostParams) {
  try {
    const { content, frontmatter } = await getBlogPost('difference-between-const-var-and-let');
    const typedFrontmatter = frontmatter as unknown as BlogFrontmatter;
