...
export async function generateMetadata(): Promise<Metadata> {
  try {
    const { frontmatter } = await getBlogPost('what-is-a-string-in-javascript');
    ...
    return createMetadata({
      ...
      canonicalUrl: `/javascript-fundamentals/what-is-a-string-in-javascript`,
    });
  } catch {
    return {
...
export default async function WhatIsAStringInJavascript({ params }: BlogPostParams) {
  try {
    const { content, frontmatter } = await getBlogPost('what-is-a-string-in-javascript');
    const typedFrontmatter = frontmatter as unknown as BlogFrontmatter;
