...
export async function generateMetadata(): Promise<Metadata> {
  try {
    const { frontmatter } = await getBlogPost('how-to-write-comments-in-javascript');
    ...
    return createMetadata({
      ...
      canonicalUrl: `/javascript-fundamentals/how-to-write-comments-in-javascript`,
    });
  } catch {
    return {
...
export default async function HowToUseJavaScriptComments({ params }: BlogPostParams) {
  try {
    const { content, frontmatter } = await getBlogPost('how-to-write-comments-in-javascript');
    const typedFrontmatter = frontmatter as unknown as BlogFrontmatter;
