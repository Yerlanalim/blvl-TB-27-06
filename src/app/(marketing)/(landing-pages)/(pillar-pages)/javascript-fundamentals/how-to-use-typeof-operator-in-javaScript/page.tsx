...
export async function generateMetadata(): Promise<Metadata> {
  try {
    const { frontmatter } = await getBlogPost('how-to-use-typeof-operator-in-javaScript');
    ...
    return createMetadata({
      ...
      canonicalUrl: `/javascript-fundamentals/how-to-use-typeof-operator-in-javaScript`,
    });
  } catch {
    return {
...
export default async function HowToUseTypeOfOperatorInJavaScript({ params }: BlogPostParams) {
  try {
    const { content, frontmatter } = await getBlogPost('how-to-use-typeof-operator-in-javaScript');
    const typedFrontmatter = frontmatter as unknown as BlogFrontmatter;
