...
export async function generateMetadata(): Promise<Metadata> {
  try {
    const { frontmatter } = await getBlogPost('javascript-coding-test');
    ...
    return createMetadata({
      ...
      canonicalUrl: `/javascript-coding-interview-questions-and-answers/javascript-coding-test`,
    });
  } catch {
    return {
...
export default async function JavascriptCodingTest({ params }: BlogPostParams) {
  try {
    const { content, frontmatter } = await getBlogPost('javascript-coding-test');
    ...
    return (
      <div className="container flex flex-col md:flex-row gap-10 max-w-7xl mx-auto pt-32 pb-20">
        <article className="w-full md:w-3/5">
          ...
          <div className="mb-8">
            <Link
              href="/javascript-coding-interview-questions-and-answers"
              className="flex items-center gap-x-3 hover:text-gray-400 duration-300"
            >
              <ChevronLeft size={16} />
              Back to JavaScript Coding Interview Questions and Answers
            </Link>
